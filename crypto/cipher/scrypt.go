...
func (s *Scrypt) scryptEncrypt(data []byte, passphrase []byte, N, r, p int) (*cryptoJSON, error) {
	salt := common.RandomCSPRNG(ScryptDKLen)
	...
	iv := common.RandomCSPRNG(aes.BlockSize) // 16
	cipherText, err := s.aesCTRXOR(encryptKey, data, iv)
	if err != nil {
		return nil, err
	}

	mac := common.Sha256(derivedKey[16:32], cipherText, iv, []byte(cipherName))
...
func (s *Scrypt) scryptDecrypt(crypto *cryptoJSON, passphrase []byte, version int) ([]byte, error) {
	...
	if version == currentVersion {
		calculatedMAC = common.Sha256(derivedKey[16:32], cipherText, iv, []byte(crypto.Cipher))
	} else if version == version3 {
		calculatedMAC = common.Sha256(derivedKey[16:32], cipherText)
