...
func NewTree(tree_size uint64, hashes []common.Hash, store HashStore) *CompactMerkleTree {
...
func (self *CompactMerkleTree) Hashes() []common.Hash {
	return self.hashes
}
...
func (self *CompactMerkleTree) Marshal() ([]byte, error) {
	length := 8 + len(self.hashes)*common.HashLength
...
func (self *CompactMerkleTree) UnMarshal(buf []byte) error {
	...
	if len(buf) < 8+int(nhashes)*common.HashLength {
		return errors.New("Too short input buf length")
	}
	hashes := make([]common.Hash, nhashes, nhashes)
	for i := 0; i < int(nhashes); i++ {
		copy(hashes[i][:], buf[8+i*common.HashLength:])
	}
	self._update(tree_size, hashes)
	return nil
}

func (self *CompactMerkleTree) _update(tree_size uint64, hashes []common.Hash) {
...
func (self *CompactMerkleTree) Root() common.Hash {
...
func (self *CompactMerkleTree) GetRootWithNewLeaf(newLeaf common.Hash) common.Hash {
...
func (self *CompactMerkleTree) Append(leafv []byte) []common.Hash {
...
func (self *CompactMerkleTree) AppendHash(leaf common.Hash) []common.Hash {
	size := len(self.hashes)
	auditPath := make([]common.Hash, size, size)
	storehashes := make([]common.Hash, 0)
	// reverse
...
func (self *CompactMerkleTree) Subtract() {

}

func (self *CompactMerkleTree) DumpStatus() {
	log.Error("tree root:", self.rootHash)
	log.Error("tree size:", self.treeSize)
	log.Error("hashes size:", len(self.hashes))
	log.Error("hashes  ", self.hashes)
	log.Error("mintree_h  ", self.mintree_h)
}
...
func (self *CompactMerkleTree) merkleRoot(n uint64) common.Hash {
...
func (self *CompactMerkleTree) ConsistencyProof(m, n uint64) []common.Hash {
...
// m, n 1-based
func (self *CompactMerkleTree) subproof(m, n uint64, b bool) []common.Hash {
	offset := uint64(0)
	...
	for m < n {
		...
		if m <= k {
			...
			subhashes := make([]common.Hash, len(pos), len(pos))
			for p := range pos {
				pos[p] += offset + k*2 - 1
	...
	length := len(hashes)
	reverse := make([]common.Hash, length, length)
...
func (self *CompactMerkleTree) InclusionProof(m, n uint64) ([]common.Hash, error) {
	if m >= n {
...
*/
func (self *MerkleVerifier) VerifyLeafHashInclusion(leaf_hash common.Hash,
	leaf_index uint64, proof []common.Hash, root_hash common.Hash, tree_size uint64) error {
...
func (self *MerkleVerifier) calculate_root_hash_from_audit_path(leaf_hash common.Hash,
	node_index uint64, audit_path []common.Hash, tree_size uint64) (common.Hash, error) {
...
func (self *MerkleVerifier) VerifyConsistency(old_tree_size,
	new_tree_size uint64, old_root, new_root common.Hash, proof []common.Hash) error {
	...
	*/
	...
	lenp := len(proof)
	pos := 0
	var new_hash, old_hash common.Hash
