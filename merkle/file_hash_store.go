...
type HashStore interface {
	Append(hash []common.Hash) error
	//Truncate(pos uint64) error
	Flush() error
	Close()
	GetHash(pos uint64) (common.Hash, error)
}
...
func NewFileHashStore(name string, tree_size uint64) (HashStore, error) {
	...
	size := int64(num_hashes) * int64(common.HashLength)
...
func (self *fileHashStore) checkConsistence(tree_size uint64) error {
	...
	if err != nil {
		return err
	} else if stat.Size() < int64(num_hashes)*int64(common.HashLength) {
		return errors.New("stored hashes are less than expected")
	}
	return nil
}

func (self *fileHashStore) Append(hash []common.Hash) error {
	if self == nil {
		return nil
	}
	buf := make([]byte, 0, len(hash)*common.HashLength)
	for _, h := range hash {
		buf = append(buf, h[:]...)
	}
...
func (self *fileHashStore) Truncate(pos uint64) error {
	if self == nil {
		return nil
	}
	err := self.file.Truncate(int64(pos))
	return err
}
...
func (self *fileHashStore) GetHash(pos uint64) (common.Hash, error) {
	if self == nil {
		return common.Hash{}, errors.New("FileHashstore is nil")
	}
	hash := common.Hash{}
	_, err := self.file.ReadAt(hash[:], int64(pos)*int64(common.HashLength))
	if err != nil {
...
func (self *memHashStore) Append(hash []common.Hash) error {
	self.hashes = append(self.hashes, hash...)
	return nil
}

func (self *memHashStore) GetHash(pos uint64) (common.Hash, error) {
