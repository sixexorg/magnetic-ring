...
func LVSerialize(lv LValue) []byte {

	sk := sink.NewZeroCopySink(nil)
	switch lv := lv.(type) {
	case LNumber:
		bits := math.Float64bits(float64(lv))
		sk.WriteByte(byte(LTNumber))
		sk.WriteUint64(bits)
	case LString:
		sk.WriteByte(byte(LTString))
		sk.WriteString(string(lv))
	case LBool:
		sk.WriteByte(byte(LTBool))
		sk.WriteBool(bool(lv))
	case *LTable:
		// mark type
		sk.WriteByte(byte(LTTable))
		// write Metatable
		sk.WriteVarBytes(LVSerialize(lv.Metatable))
		// write array
		length := len(lv.array)
		sk.WriteVarUint(uint64(length))
		for _, value := range lv.array {
			sk.WriteVarBytes(LVSerialize(value))
		}
		// write dict
		length = len(lv.dict)
		sk.WriteVarUint(uint64(length))
		for k, v := range lv.dict {
			sk.WriteVarBytes(LVSerialize(k))
			sk.WriteVarBytes(LVSerialize(v))
		}
		// write strdict
		length = len(lv.strdict)
		sk.WriteVarUint(uint64(length))
		for k, v := range lv.strdict {
			sk.WriteString(k)
			sk.WriteVarBytes(LVSerialize(v))
		}
		// write keys
		length = len(lv.keys)
		sk.WriteVarUint(uint64(length))
		for _, value := range lv.keys {
			sk.WriteVarBytes(LVSerialize(value))
		}
		// write k2i
		length = len(lv.k2i)
		sk.WriteVarUint(uint64(length))
		for k, v := range lv.k2i {
			sk.WriteVarBytes(LVSerialize(k))
			sk.WriteInt64(int64(v))
		}
	}
	return sk.Bytes()
}

func LVDeserialize(bytes []byte) LValue {
	source := sink.NewZeroCopySource(bytes)
	// index := 0
	t, _ := source.NextByte()
	switch t {
	case byte(LTNumber):
		bits, _ := source.NextUint64()
	...
	case byte(LTString):
		...
		return LString(str)
	case byte(LTBool):
		b, _, _ := source.NextBool()
		return LBool(b)
	case byte(LTTable):
		b, _, _, _ := source.NextVarBytes()
		metaTable := LVDeserialize(b)

		length, _, _, _ := source.NextVarUint()
		array := make([]LValue, length)
		for i := 0; i < int(length); i++ {
			b, _, _, _ = source.NextVarBytes()
			array[i] = LVDeserialize(b)
		}

		length, _, _, _ = source.NextVarUint()
		dict := make(map[LValue]LValue, length)
		for i := 0; i < int(length); i++ {
			bk, _, _, _ := source.NextVarBytes()
			bv, _, _, _ := source.NextVarBytes()
			dict[LVDeserialize(bk)] = LVDeserialize(bv)
		}

		length, _, _, _ = source.NextVarUint()
		strdict := make(map[string]LValue, length)
		for i := 0; i < int(length); i++ {
			k, _, _, _ := source.NextString()
			v, _, _, _ := source.NextVarBytes()
			strdict[k] = LVDeserialize(v)
		}

		length, _, _, _ = source.NextVarUint()
		keys := make([]LValue, length)
		for i := 0; i < int(length); i++ {
			b, _, _, _ = source.NextVarBytes()
			keys[i] = LVDeserialize(b)
		}

		length, _, _, _ = source.NextVarUint()
		k2i := make(map[LValue]int, length)
		for i := 0; i < int(length); i++ {
			k, _, _, _ := source.NextVarBytes()
			v, _ := source.NextInt64()
			k2i[LVDeserialize(k)] = int(v)
		}
		return &LTable{
			Metatable: metaTable,
			array:     array,
