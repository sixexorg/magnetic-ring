...
func NewSubTxQueue() *SubTxQueue {
	queue := new(SubTxQueue)
...
*/
func (queue *SubTxQueue) Enqueue(tx *types.Transaction) {
	queue.Lock()
	defer queue.Unlock()
	qt := newQueueTx(tx)
...
*/
func (queue *SubTxQueue) Dequeue() *types.Transaction {
	if len(queue.txs) < 1 {
		return nil
	}
	queue.Lock()
...
func (queue *SubTxQueue) Remove(hash common.Hash) *types.Transaction {
	queue.Lock()
	defer queue.Unlock()
...
*/
func (q *SubTxQueue) IsEmpty() bool {
	return len(q.txs) == 0
}
