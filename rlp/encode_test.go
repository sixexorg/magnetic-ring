...
var txAmount TxAmounts

type TxAmounts struct {
	Tss []*TxAmount
}

func TestAAAA(t *testing.T) {
	buff := new(bytes.Buffer)
	err := Encode(buff, txAmount)
	t.Log(err)
	t.Log(len(buff.Bytes()))
	t.Log(buff.Bytes())
	var fs TxAmounts
	err = Decode(bytes.NewReader(buff.Bytes()), &fs)
	t.Log(err)
	for _, v := range fs.Tss {

		t.Log(v.Address.ToString(), v.Amount.String())
	}
