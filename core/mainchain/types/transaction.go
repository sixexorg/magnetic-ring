...
func NewTransaction(typ TransactionType, version byte, txData *TxData) (*Transaction, error) {
	tx := &Transaction{
		Version: version,
		TxType:  typ,
		TxData:  txData,
...
func (tx *Transaction) Hash() common.Hash {
	raw, _ := tx.getHashRaw()
	tx.TransactionHash, _ = common.ParseHashFromBytes(common.Sha256(raw, tx.Sigs.ToBytes()))
		...
		for _, unit := range *lst {
			...
			for _, pbuf := range unit.Pubks {
				...
				for _, sig := range *layer {
					addr, err := account.PubkToAddress(pubkey)
					if err != nil {
						fmt.Printf("account . pubktoaddress error=%v\n", err)
						continue
					}
					fmt.Printf("printpubkaddress=%v\n", addr.ToString())
