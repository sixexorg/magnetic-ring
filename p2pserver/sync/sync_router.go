...
func (this *MessageRouter) init(p2p p2p.P2P) {
	...
	this.RegisterMsgHandler(msgCommon.PINGSPC_TYPE, PingSpcHandle)
	this.RegisterMsgHandler(msgCommon.PONGSPC_TYPE, PongSpcHandle)

	this.RegisterMsgHandler(msgCommon.EARTHNOTIFYHASH, EarthNotifyHashHandle)
	this.RegisterMsgHandler(msgCommon.NODE_LEAGUE_HEIGHT, NodeLeagueHeigtNtfHandle)

	this.RegisterMsgHandler(msgCommon.EXTDATA_REQ_TYPE, ExtDataRequestHandle)
	this.RegisterMsgHandler(msgCommon.EXTDATA_RSP_TYPE, ExtDataResponseHandle)
		...
		case data, ok := <-channel:
			if ok {
				...
				if ok {
					...
					log.Warn("unknown message handler for the msg", "msgType", msgType)
				}
			}
		case <-stopCh:
			return
