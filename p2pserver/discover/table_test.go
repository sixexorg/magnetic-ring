...
func TestTable_pingReplace(t *testing.T) {
	doit := func(newNodeIsResponding, lastInBucketIsResponding bool) {
		...
		tab, _ := newTable(transport, NodeID{}, &net.UDPAddr{}, "",false)
		defer tab.Close()
...
func (*pingRecorder)	findcircle(toid NodeID, addr *net.UDPAddr, 
	target comm.Address) ([]*Node, error) {return make([]*Node,0),nil}
func (*pingRecorder) sendcircle(ndoe,srcnode *Node,SrcCircleID comm.Address,
	ownid uint64,badd bool) error {return nil}
func (*pingRecorder) sendConnectInfo(ndoe,srcnode *Node,ownid,
	remoteid uint64,bconnect,bstellar bool) error {return nil}
func (*pingRecorder) reqPeerOrg(toid NodeID, toaddr *net.UDPAddr,
	node *Node) (*rtnTabPeerOrg, error) {return &rtnTabPeerOrg{},nil}
func (*pingRecorder) reqPeerConnect(toid NodeID, toaddr *net.UDPAddr,
	node *Node) (*rtnTabPeerConnect, error) {return &rtnTabPeerConnect{},nil}
func (*pingRecorder) connectCircuOrg(ndoe,connectnode *Node,
	orgid comm.Address) error {return nil}
...
func TestTable_closest(t *testing.T) {
	...
	test := func(test *closeTest) bool {
		...
		tab, _ := newTable(nil, test.Self, &net.UDPAddr{}, "",false)
		defer tab.Close()
		tab.stuff(test.All)
...
func TestTable_ReadRandomNodesGetAll(t *testing.T) {
	...
	test := func(buf []*Node) bool {
		tab, _ := newTable(nil, NodeID{}, &net.UDPAddr{}, "",false)
		defer tab.Close()
		for i := 0; i < len(buf); i++ {
...
func TestTable_Lookup(t *testing.T) {
	...
	tab, _ := newTable(lookupTestnet, self.ID, &net.UDPAddr{}, "",false)
	defer tab.Close()
...
func (*preminedTestnet)	findcircle(toid NodeID, addr *net.UDPAddr, 
	target comm.Address) ([]*Node, error) {return make([]*Node,0),nil}
func (*preminedTestnet) sendcircle(ndoe,srcnode *Node,SrcCircleID comm.Address,
	ownid uint64,badd bool) error {return nil}
func (*preminedTestnet) sendConnectInfo(ndoe,srcnode *Node,ownid,
	remoteid uint64,bconnect,bstellar bool) error {return nil}
func (*preminedTestnet) reqPeerOrg(toid NodeID, toaddr *net.UDPAddr,
	node *Node) (*rtnTabPeerOrg, error) {return &rtnTabPeerOrg{},nil}
func (*preminedTestnet) reqPeerConnect(toid NodeID, toaddr *net.UDPAddr,
	node *Node) (*rtnTabPeerConnect, error) {return &rtnTabPeerConnect{},nil}
func (*preminedTestnet) connectCircuOrg(ndoe,connectnode *Node,
	orgid comm.Address) error {return nil}
// func (*preminedTestnet) close()
...
func TestTable_singleOrgCheck(t *testing.T) {
	tab, _ := newTable(nil, NodeID{}, &net.UDPAddr{}, "",false)
	defer tab.Close()

	orgid,err := comm.AddressFromHexString("abcabcabcabcabcabcabcabcabcabcabcaaabbcc")
	if err == nil {
		// this.AddOrg(stellarnodeid)
		fmt.Println("[TestTable_singleOrgCheck] orgid:",orgid)
	}else{
		return
	}
	orgpeer := make(NodeMap)
	orgpeer[1] = true
	orgpeer[2] = true
	orgpeer[3] = true
	orgpeer[4] = true
	orgpeer[5] = true
	tab.orgData.orgSingleData[orgid] = orgpeer

	peertopeer1 := make(ConnPeer)
	peertopeer1[2] = true
	peertopeer1[3] = true
	peertopeer2 := make(ConnPeer)
	peertopeer2[1] = true
	peertopeer2[3] = true
	peertopeer3 := make(ConnPeer)
	peertopeer3[2] = true
	peertopeer3[1] = true

	peertopeer4 := make(ConnPeer)
	peertopeer4[5] = true
	peertopeer5 := make(ConnPeer)
	peertopeer5[4] = true

	tab.connectData.peerToPeerMap[1]=peertopeer1
	tab.connectData.peerToPeerMap[2]=peertopeer2
	tab.connectData.peerToPeerMap[3]=peertopeer3
	tab.connectData.peerToPeerMap[4]=peertopeer4
	tab.connectData.peerToPeerMap[5]=peertopeer5

	circu := tab.singleOrgCheck(orgid)
	if len(circu) > 1 {
		fmt.Println(" TestTable_singleOrgCheck circu:",circu)
	}
}