...
import (
	...
	berrors "github.com/sixexorg/magnetic-ring/errors"
)
...
func (self *LevelDBStore) Get(key []byte) ([]byte, error) {
	...
	if err != nil {
		if err == leveldb.ErrNotFound {
			return nil, berrors.ERR_DB_NOT_FOUND
		}
		return nil, err
	}
	return dat, nil
...
func (self *LevelDBStore) NewIterator(prefix []byte) *Iterator {
...
func (self *LevelDBStore) NewSeniorIterator(p *util.Range) *Iterator {
	iter := self.db.NewIterator(p, nil)
	return &Iterator{
		iter: iter,
	}
}
